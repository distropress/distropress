...
$table_usermeta = array(
	'group_id'         => 'fluxbb-group_id',
	'salt'             => 'fluxbb-salt',
	'title'            => 'distropress-title',
	'realname'         => array('first_name', 'last_name'),
	'facebook'         => 'distropress-facebook',
	'twitter'          => 'distropress-twitter',
	'linkedin'         => 'distropress-linkedin',
	'skype'            => 'distropress-facebook',
	'jabber'           => 'jabber',
	'icq'              => 'distropress-icq',
	'msn'              => 'distropress-msn',
	'aim'              => 'aim',
	'yahoo'            => 'yim',
	'location'         => 'distropress-location',
	'signature'        => 'distropress-signature',
	'disp_topics'      => 'fluxbb-disp_topics',
	'disp_posts'       => 'fluxbb-disp_posts',
	'email_setting'    => 'fluxbb-email_setting',
	'notify_with_post' => 'fluxbb-notify_with_post',
	'auto_notify'      => 'fluxbb-auto_notify',
	'show_smilies'     => 'fluxbb-show_smilies',
	'show_img'         => 'fluxbb-show_img',
	'show_img_sig'     => 'fluxbb-show_img_sig',
	'show_avatars'     => 'fluxbb-show_avatars',
	'show_sig'         => 'fluxbb-show_sig',
	'access_keys'      => 'fluxbb-access_keys',
	'timezone'         => 'distropress-timezone',
	'dst'              => 'distropress-dst',
	'time_format'      => 'distropress-time_format',
	'date_format'      => 'distropress-date_format',
	'language'         => 'fluxbb-language',
	'style'            => 'fluxbb-style',
	'num_posts'        => 'fluxbb-num_posts',
	'last_post'        => 'fluxbb-last_post',
	'last_search'      => 'fluxbb-last_search',
	'last_email_sent'  => 'fluxbb-last_email_sent',
	'last_report_sent' => 'fluxbb-last_report_sent',
	'registered'       => 'distropress-registered',
	'registration_ip'  => 'distropress-registration_ip',
	'last_visit'       => 'fluxbb-last_visit',
	'admin_note'       => 'fluxbb-admin_note',
	'activate_string'  => 'fluxbb-activate_string',
	'activate_key'     => 'fluxbb-activate_key',
	'avatar'           => 'fluxbb-avatar',
	'avatar_width'     => 'fluxbb-avatar_width',
	'avatar_height'    => 'fluxbb-avatar_height'
);

if (strstr($sql, 'SELECT') && strstr($sql, $this->prefix.'users')) {
	$o_sql = $sql; // Eliminar

	if (strstr($sql, 'u.*')) {
		$sql = str_replace('u.*', implode(', ', array_keys(array_merge($table_users, $table_usermeta))), $sql);
	} else {
		$sql = str_replace('*', implode(', ', array_keys(array_merge($table_users, $table_usermeta))), $sql);
		$sql = str_replace('u.', '', $sql);
	}

	preg_match('~^(?:SELECT\s)(.*)(?:\sFROM)~U', $sql, $matches);
	...
	foreach ($columns as $key => $column) {
		if (in_array($column, array_keys($table_users)) && $columns[$key] != $table_users[$column] && $column != 'id') {
			$columns[$key] = 'u.'.$table_users[$column].' AS '.$columns[$key];
		} elseif (in_array($column, array_keys($table_usermeta)) && $column != 'realname') {
			$columns[$key] = 'MAX(IF(um.meta_key = \''.$table_usermeta[$column].'\', um.meta_value, NULL)) AS '.$column;
			$usermeta = true;
		} elseif ($column == 'id') {
			$columns[$key] = 'u.ID + 1 AS id';
		} elseif ($column == 'realname') {
			$columns[$key] = 'CONCAT(MAX(IF(um.meta_key = \'first_name\', um.meta_value, NULL)), \' \', MAX(IF(um.meta_key = \'last_name\', um.meta_value, NULL))) AS realname';
			$usermeta = true;
		} elseif (!strstr($column, '.')) {
			$columns[$key] = 'u.'.$column;
		}
	}

	$sql = str_replace($matches[0], 'SELECT '.implode(', ', $columns).' FROM', $sql);

	if ($usermeta == true) {
		$sql = str_replace($this->prefix.'groups AS g ON u.group_id=g.g_id', 'wp_fluxbb_groups AS g ON (SELECT meta_value FROM wp_usermeta WHERE user_id=id AND meta_key=\'fluxbb-group_id\')=g.g_id', $sql);
		$sql = str_replace($this->prefix.'users AS u ', 'wp_users AS u LEFT JOIN wp_usermeta AS um ON um.user_id=id ', $sql);
	}
	if (strstr($sql, 'g.g_id=group_id'))
		$sql = str_replace('g.g_id=group_id', 'g.g_id=(um.meta_key=\'fluxbb-group_id\')', $sql);
	if (strstr($sql, 'WHERE u.id=') || strstr($sql, 'WHERE id='))
		$sql = $sql.'-1';
	// Desde: SELECT COUNT(id) FROM wp_fluxbb_users AS u WHERE u.id>1 AND u.group_id!=0
	// Hasta: SELECT COUNT(user_id) FROM wp_usermeta AS um WHERE um.meta_key = 'fluxbb-group_id' AND um.meta_value != 0
	if (!strstr($sql, 'r.id'))
		$sql = $sql.' GROUP BY um.user_id';
	if (strstr($sql, 'u.username'))
		$sql = str_replace('u.username', 'u.user_login', $sql);
	if (strstr($sql, 'u2.username'))
		$sql = str_replace('u2.username', 'u2.user_login', $sql);
	if (strstr($sql, 'r.reported_by=id'))
		$sql = str_replace('r.reported_by=id', 'r.reported_by=u.ID+1', $sql);
	if (strstr($sql, 'r.zapped_by=u2.id'))
		$sql = str_replace('r.zapped_by=u2.id', 'r.zapped_by=u2.ID+1', $sql);
	if (strstr($sql, $this->prefix.'users '))
		$sql = str_replace($this->prefix.'users ', 'wp_users ', $sql);

	/* Eliminar */
	//echo ' <tt style="color:green">'.$o_sql.'</tt>';
	//echo ' <tt style="color:red">'.$sql.'</tt>';
	//$sql = $o_sql;
	/* Eliminar */
}
